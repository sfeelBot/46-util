...
| filename_matching | 이물검사 이미지 파일명을 바코드→셀번호→재료명 순으로 재가공하는 PyQt5 GUI. 폴더 재귀 스캔·확장자 필터·최종명 미리보기·중복검사·우클릭 탐색기 열기·비동기 일괄 변환(원본 보존, 복사만)·되돌리기(로그 기반) 제공. 매핑표는 외부 CSV로 분리 | [utils/filename_matching/processing.md](utils/filename_matching/processing.md) |
