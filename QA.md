...
| github_sync_gui | [utils/github_sync_gui/QA.md](utils/github_sync_gui/QA.md) | robocopy ExitCode=16(공백 경로 인용 오류)·로그 인코딩 깨짐·GUI 블로킹(비동기화)·robocopy `/MIR`의 로컬 파일 삭제 등 수정 완료. robocopy가 잠긴 파일 앞에서 무한 대기하던 문제를 재시도 제한(`/R:3 /W:10`) + "강제 동기화 취소" 버튼(`taskkill /T`)으로 해결, 스케줄 기반 자동 동기화 기능은 전체 삭제. 단계별 소요 시간 로그 + 완료 팝업 추가, 서브에이전트 검증 통과 (2026-07-13) |
