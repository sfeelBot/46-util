...
**탭으로 여러 GitHub 저장소를 각각 다른 폴더에 등록해 한 번에 관리**할 수 있다. 탭마다 지금 바로 동기화(실행 중 강제 취소 가능), 마지막 동기화 상태·로그 확인, 프로젝트 경로(DestDir)/상태 경로(StateDir) 설정을 독립적으로 처리한다.
...
1. 처음 실행하면 `%LOCALAPPDATA%\46util-sync\`에 스크립트/설정이 자동 생성되고, 탭 1개(기본 저장소)로 시작한다.
2. 탭 안의 "저장소 설정"/"경로 설정"에서 동기화할 저장소와 프로젝트 폴더(DestDir)를 지정하고 저장한다.
3. 탭 목록 오른쪽 위 "+ 저장소 추가" 버튼으로 새 저장소용 탭을 추가할 수 있다 (자유롭게 추가/삭제 가능, 탭을 우클릭하면 "삭제"가 나온다 — 삭제해도 이미 반영된 파일과 로그는 남는다).
4. 탭 안의 "지금 바로 동기화"로 그 탭의 저장소만 즉시 반영한다. 대상 폴더의 파일(예: 열려있는 csv)이 다른 프로그램에 잠겨 있어 오래 걸리는 경우 "강제 동기화 취소"로 즉시 중단할 수 있다. 끝나면 완료/실패/취소 여부를 알려주는 팝업이 뜬다. 어느 단계가 오래 걸렸는지는 로그에서 확인할 수 있다(단계별 소요 시간이 자동 기록됨).
5. 창을 닫아도(X 버튼) 프로그램은 종료되지 않고 트레이에 상주한다.
   - 트레이 아이콘을 클릭/더블클릭하면 창이 다시 열린다.
   - 트레이 아이콘을 우클릭하면 "지금 동기화 (전체)"(등록된 모든 탭을 순차 동기화), "창 열기", "종료" 메뉴가 나온다.
   - 완전히 끄려면 반드시 트레이 메뉴의 "종료"를 사용한다 (X 버튼은 트레이로 숨기기만 함).
6. "Windows 시작 시 자동 실행"을 켜면 컴퓨터를 껐다 켜서 로그인할 때 이 프로그램(트레이 상주, 등록된 모든 탭 포함)이 자동으로 실행된다 (exe로 빌드해서 사용하는 것을 전제로 함).
7. 탭별로 로그가 길어지면 그 탭의 "로그 삭제"로 `sync.log`를 지울 수 있다 (동기화 동작/설정에는 영향 없음).

exe로 빌드해서 배포하려면 (결과물: `dist\github-sync-feel.exe`):
...
**`gui_folder_remap.py`(완전 독립 실행, gui.py/core.py 비의존)**: 파일명 안의 저장번호(`Test#A../Test#B..`)를 매핑표(`storage_ab_defect_info.csv`, GitHub 이슈 #5 데이터)와 직접 매칭해 `{이물정보}_{셀번호}_{원본파일명}`으로 재명명한다. 셀번호를 모르는 경우(`NULL`)도 이물정보만으로 정상 변환된다.
...
**사용법 (`gui_folder_remap.py`: 이슈#5 매핑표 기반 파일명 재명명)**

```bash
.venv\Scripts\python.exe utils\filename_matching\gui_folder_remap.py
```

1. 대상 폴더 선택 (하위 폴더 재귀 스캔, 폴더 구조 제약 없음 — 파일명만 봄)
2. 시작 시 `mapping/storage_ab_defect_info.csv`가 기본 매핑으로 로드됨
3. 확장자 선택 후 "목록 불러오기" — 각 파일명에서 저장번호(`Test#A../Test#B..`)를 찾아 매핑표에서 셀번호+이물정보를 조회하고 `{이물정보}_{셀번호}_{원본파일명}`으로 재명명. 셀번호가 `NULL`인 행(바코드 매칭 실패로 셀을 모르는 경우)도 이물정보만으로 정상 변환됨. 저장번호 패턴이 없거나 매핑표에 아예 없으면 "매칭실패"
4. 이후 흐름(중복검사/우클릭 탐색기/출력 폴더·모드/모두 변환/되돌리기)은 탭1/탭2와 같은 방식으로 동작하지만 이 파일 안에 독립적으로 구현되어 있음 (error 폴더 자동 백업은 없음)

### folder_suffix_copier

지정 폴더를 재귀 탐색해 파일(bmp/raw 기본, 스캔된 확장자 중 체크박스로 선택)에 **상위 폴더명들을 `_`로 이어붙인 접미어**를 파일명 뒤(확장자 앞)에 붙인 사본을 만드는 PyQt5 GUI 도구. 원본은 절대 수정/이동하지 않는다(복사만).
예: 대상 폴더가 `E:\data`일 때 `E:\data\LotA\Cell1\img.bmp` → `img_LotA_Cell1.bmp`
자세한 내용은 [utils/folder_suffix_copier/processing.md](utils/folder_suffix_copier/processing.md) 참고.
...
1. "찾아보기…"로 대상 폴더를 지정하면 출력 폴더 기본값으로 `<대상폴더>_renamed`가 자동 제안됨
2. 저장 방식 선택: "별도 출력 폴더에 모으기"(기본) 또는 "원본과 같은 폴더에 생성"
3. "스캔" 버튼 → 하위 폴더 재귀 탐색 후 발견된 확장자들이 체크박스로 표시됨 (`.bmp`/`.raw`는 기본 체크, 다른 확장자도 체크하면 대상에 포함)
4. 테이블에서 `상대경로 | 변경 전 | 변경 후` 미리보기 확인. 대상 폴더 바로 아래 파일은 접미어 없이 원본명 그대로이며, 이름 충돌로 `_2`, `_3` 번호가 붙는 파일은 빨간색으로 표시됨
5. "실행 (복사)" → 확인 팝업 후 백그라운드에서 복사 진행. 하단 로그창에 파일마다 `변경 전 → 변경 후`가 출력되고 완료 시 성공/실패 건수가 요약됨

### feel_capture (필캡쳐)

시스템 트레이에 상주하며 단축키 한 번으로 화면을 캡쳐/녹화하는 PyQt5 도구.
드래그 방식(화면이 어두워지고 드래그한 영역만 밝게 표시) / 영역 방식(빨간 고정 박스, 상단 손잡이로 이동·가장자리로 리사이즈, 우클릭으로 프리셋 3종·사용자 지정 크기·고정 토글) 중 모드를 고를 수 있고, 클립보드 복사 또는 타임스탬프 파일명으로 폴더 저장(png/jpg/bmp/webp/tiff 정지 이미지 + gif/mp4/avi 실제 녹화), 저장 시 리사이즈(고정 크기/비율/**엑셀 셀 크기에 맞춤** — 픽셀은 원본 그대로 두고 DPI 정보만 조정해 엑셀/워드에 삽입될 때만 지정한 셀 크기로 보이게 함, 비율 안 깨짐, png/jpg/tiff 전용), 사용자 지정 전역 단축키(녹화 포맷일 때는 단축키가 시작/종료 토글로 동작), 로그 파일(문제 재현 시 GitHub issue에 붙여넣어 진단용), 멀티 모니터를 지원한다.
자세한 내용은 [utils/feel_capture/processing.md](utils/feel_capture/processing.md) 참고.
...
1. 실행하면 시스템 트레이에 카메라 모양 아이콘이 뜬다. 트레이 아이콘을 클릭(좌클릭/더블클릭)하거나 우클릭 → "설정..."을 선택하면 설정 다이얼로그가 열린다. 모드(드래그/영역), 저장 방식(클립보드/출력 폴더), 확장자, 저장 시 리사이즈(엑셀 셀 크기 맞춤 포함), 단축키, 로그 폴더, 영역 프리셋 3종을 지정한다.
2. 지정한 단축키를 누르면 현재 선택된 모드로 캡쳐가 실행된다. 확장자가 gif/mp4/avi(녹화 포맷)이면 첫 단축키 입력으로 녹화가 시작되고(화면에 "● REC" 표시), 다시 누르면 종료·저장된다.
3. 영역 모드에서는 화면의 빨간 박스 상단의 굵은 손잡이 부분을 잡고 끌면 이동, 가장자리(하단/좌우/모서리)를 끌면 크기 조절이 된다. 박스를 우클릭하면 프리셋 크기/사용자 지정 크기/고정 토글/설정 열기 메뉴가 뜬다.
4. 캡쳐를 엑셀에 붙여넣기 좋은 크기로 만들려면 설정의 리사이즈에서 "엑셀 셀 크기에 맞춤"(png/jpg/tiff에서만 선택 가능)을 선택하고 엑셀에서 보이는 열너비(문자 수)/행높이(포인트) 값을 그대로 입력한다. 실제 픽셀(원본 해상도)은 전혀 건드리지 않고 DPI 정보만 조정하므로, 엑셀/워드에 삽입될 때만 그 셀 크기 안에 비율 그대로 작게 보이고, 나중에 다시 확대해도 화질이 떨어지지 않는다(DPI를 읽지 않는 프로그램에서는 원본 크기 그대로 보일 수 있음).
5. 완전히 종료하려면 트레이 아이콘 우클릭 → "종료" (창을 닫는 개념이 없는 트레이 전용 앱).
6. 문제가 생기면 설정의 "로그 폴더"(기본값 `%APPDATA%\FeelCapture\logs`)의 `feel_capture.log` 파일을 GitHub issue에 붙여넣으면 원인 진단에 도움이 된다.

exe로 빌드해서 배포하려면 (아이콘은 이미 `assets/icon.ico`로 생성되어 저장소에 포함됨):
