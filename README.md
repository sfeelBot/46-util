...
드래그 방식(화면이 어두워지고 드래그한 영역만 밝게 표시) / 영역 방식(빨간 고정 박스, 드래그로 이동·리사이즈, 우클릭으로 프리셋 3종·사용자 지정 크기·고정 토글) 중 모드를 고를 수 있고, 클립보드 복사 또는 타임스탬프 파일명으로 폴더 저장(png/jpg/bmp/webp/tiff 정지 이미지 + gif/mp4/avi 실제 녹화), 저장 시 리사이즈, 사용자 지정 전역 단축키(녹화 포맷일 때는 단축키가 시작/종료 토글로 동작), 멀티 모니터를 지원한다.
...
1. 실행하면 시스템 트레이에 카메라 모양 아이콘이 뜬다. 우클릭 → "설정..."에서 모드(드래그/영역), 저장 방식(클립보드/출력 폴더), 확장자, 저장 시 리사이즈, 단축키, 영역 프리셋 3종을 지정한다.
...
4. 완전히 종료하려면 트레이 아이콘 우클릭 → "종료" (창을 닫는 개념이 없는 트레이 전용 앱).
