...
3. "지금 바로 동기화"로 즉시 반영한다. 대상 폴더의 파일(예: 열려있는 csv)이 다른 프로그램에 잠겨 있어 오래 걸리는 경우 "강제 동기화 취소"로 즉시 중단할 수 있다. 끝나면 완료/실패/취소 여부를 알려주는 팝업이 뜬다. 어느 단계가 오래 걸렸는지는 로그에서 확인할 수 있다(단계별 소요 시간이 자동 기록됨).
...
4. 캡쳐를 엑셀에 붙여넣기 좋은 크기로 만들려면 설정의 리사이즈에서 "엑셀 셀 크기에 맞춤"을 선택하고 엑셀에서 보이는 열너비(문자 수)/행높이(포인트) 값을 그대로 입력한다. 원본 비율은 유지한 채 그 셀 크기 안에 최대로 들어가도록 축소된다.
