...
| image_cropper | 폴더 내 이미지(JPG/PNG/BMP/TIFF/RAW 16-bit)에서 복수 ROI를 드래그/숫자입력/레퍼런스 이미지 파일명(XYWH) 로드로 지정해 크롭 저장하는 PyQt5 도구. ROI 개별 선택·재지정·삭제, 하위 폴더 포함 스캔(cropped 폴더 자동 제외)·확장자 체크박스 선택 후 목록 불러오기·파일명/폴더 정렬 가능한 목록 지원 | [utils/image_cropper/processing.md](utils/image_cropper/processing.md) |
