...
.SYNOPSIS
    ...
    tools/github_sync/*.ps1 스크립트를 리소스로 함께 번들하여, exe 하나만 옮겨도
    실행 시 %LOCALAPPDATA%\46util-sync\ 에 스크립트를 풀어놓고 정상 동작하도록 한다.
...
foreach ($f in @($SyncScript, $ManageScript)) {
    if (-not (Test-Path $f)) {
        throw "필요한 스크립트를 찾을 수 없습니다: $f"
    }
}
